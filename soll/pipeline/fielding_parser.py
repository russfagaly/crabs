# fielding_parser.py — 2026 SOLL 12U All-Stars
#
# Best-effort fielding stat extraction (PO, A, DP) from the play-by-play
# logs. Only counts plays where SOLL are on defense (opponent batting)
# AND a SOLL fielder is named in the play description. Many routine
...
def parse_game_fielding(game_id):
    ...
    where SOLL were on defense and a SOLL fielder was named."""
    stats = {}
    ...
    for half, header, blocks in innings:
        ...
        crabs_batting = 'SOLL' in batting_team or 'SOLL' in batting_team
        if crabs_batting:
            continue  # SOLL at bat -> SOLL not fielding, skip
