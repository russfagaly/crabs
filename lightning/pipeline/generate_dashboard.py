#!/usr/bin/env python3
"""
generate_dashboard.py — 2026 Lafayette Lightning
...
for game_id, date_str in game_ids:
    ...
    if idx < len(gs):
        g = gs[idx]
        opp  = g['home'] if g['away'] == 'Lafayette' else g['away']
        us   = g['away_score'] if g['away'] == 'Lafayette' else g['home_score']
        them = g['home_score'] if g['away'] == 'Lafayette' else g['away_score']
        res  = 'W' if g['winner'] == 'Lafayette' else ('T' if g['winner'] == 'Tie' else 'L')
...
rs     = sum(g['away_score'] if g['away']=='Lafayette' else g['home_score'] for g in GAMES) if GAMES else 0
ra     = sum(g['home_score'] if g['away']=='Lafayette' else g['away_score'] for g in GAMES) if GAMES else 0
...
<html lang="en">
<head>
{theme.head('2026 Lafayette Lightning')}
</head>
<body>

<div class="header">
{theme.THEME_TOGGLE_BUTTON}
  <h1>2026 Lafayette Lightning</h1>
  ...
  <div style="text-align:center;margin-top:14px">
    <a href="scouting.html" style="display:inline-block;background:var(--accent);color:#000;padding:7px 16px;border-radius:6px;font-weight:700;font-size:13px;text-decoration:none">📋 Scouting Report →</a>
  </div>
</div>
...
<div class="footer">2026 Lafayette Lightning · Generated {GENERATED} · {NUM_GAMES} games</div>
</body></html>"""
...
print(f"    Games: {NUM_GAMES}  |  Record: {record_str}  |  Players: {len(ROSTER)}")
