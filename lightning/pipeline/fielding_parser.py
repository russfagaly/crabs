# fielding_parser.py — 2026 Lafayette Lightning
#
# Best-effort fielding stat extraction (PO, A, DP) from the play-by-play
# logs. Only counts plays where Lightning are on defense (opponent batting)
# AND a Lightning fielder is named in the play description. Many routine
...
def parse_game_fielding(game_id):
    ...
    where Lightning were on defense and a Lightning fielder was named."""
    stats = {}
    ...
    for half, header, blocks in innings:
        ...
        crabs_batting = 'Lightning' in batting_team or 'Lafayette' in batting_team
        if crabs_batting:
            continue  # Lightning at bat -> Lightning not fielding, skip
