# fielding_parser.py — 2026 NOLL 12U All-Stars
#
# Best-effort fielding stat extraction (PO, A, DP) from the play-by-play
# logs. Only counts plays where NOLL are on defense (opponent batting)
# AND a NOLL fielder is named in the play description. Many routine
...
def parse_game_fielding(game_id):
    ...
    where NOLL were on defense and a NOLL fielder was named."""
    stats = {}
    ...
    for half, header, blocks in innings:
        ...
        crabs_batting = 'NOLL' in batting_team or 'NOLL' in batting_team
        if crabs_batting:
            continue  # NOLL at bat -> NOLL not fielding, skip
