# fielding_parser.py — 2026 Road Dogs 12
#
# Best-effort fielding stat extraction (PO, A, DP) from the play-by-play
# logs. Only counts plays where Road Dogs are on defense (opponent batting)
# AND a Road Dogs fielder is named in the play description. Many routine
...
def parse_game_fielding(game_id):
    ...
    where Road Dogs were on defense and a Road Dogs fielder was named."""
    stats = {}
    ...
    for half, header, blocks in innings:
        ...
        crabs_batting = 'Road Dogs' in batting_team or 'Road Dogs' in batting_team
        if crabs_batting:
            continue  # Road Dogs at bat -> Road Dogs not fielding, skip
