#!/usr/bin/env python3
"""
generate_dashboard.py — 2026 Project Mill Cooperstown
...
for game_id, date_str in game_ids:
    ...
    if idx < len(gs):
        g = gs[idx]
        opp  = g['home'] if g['away'] == 'Project Mill' else g['away']
        us   = g['away_score'] if g['away'] == 'Project Mill' else g['home_score']
        them = g['home_score'] if g['away'] == 'Project Mill' else g['away_score']
        res  = 'W' if g['winner'] == 'Project Mill' else ('T' if g['winner'] == 'Tie' else 'L')
...
# ── W/L record ────────────────────────────────────────────────────────────────
wins   = sum(1 for g in GAMES if g.get('winner')=='Project Mill')
ties   = sum(1 for g in GAMES if g.get('winner')=='Tie')
losses = len(GAMES) - wins - ties
record_str = f"{wins}-{losses}" + (f"-{ties}" if ties else "")
rs     = sum(g['away_score'] if g['away']=='Project Mill' else g['home_score'] for g in GAMES) if GAMES else 0
ra     = sum(g['home_score'] if g['away']=='Project Mill' else g['away_score'] for g in GAMES) if GAMES else 0
...
<html lang="en">
<head>
{theme.head('2026 Project Mill Cooperstown')}
</head>
<body>

<div class="header">
{theme.THEME_TOGGLE_BUTTON}
  <h1>2026 Project Mill Cooperstown</h1>
...
<div class="footer">2026 Project Mill Cooperstown · Generated {GENERATED} · {NUM_GAMES} games</div>
</body></html>"""
