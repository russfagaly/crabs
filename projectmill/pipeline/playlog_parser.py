# playlog_parser.py — 2026 Project Mill Cooperstown
#
...
# Score lines are team-agnostic: any uppercase team code, e.g.
# "LFYT 9 - GRTG 18" or "LFYT 10 - GRTG 18 | 2 Outs" or just "3 Outs".
SCORE_PREFIX_RE = re.compile(r"^[A-Z]{2,6} \d+ - ")
OUTS_SCORE_RE = re.compile(r"^([A-Z]{2,6} \d+ - [A-Z]{2,6} \d+)?\s*\|?\s*(\d+ Outs?)?$")
...
def classify(line):
    ...
    if not line:
        return "BLANK"
    if OUTS_SCORE_RE.match(line) and (SCORE_PREFIX_RE.match(line) or "Out" in line):
        return "OUTS_SCORE"
